...
int main (int argc, char** argv )
{
    ...
    if (std::regex_match(filename, std::regex(".*\\.typ1$") ))
    {
        ...
        for( size_t imsh = 0; imsh < num_remesh + 1; imsh ++)
        {
            ...
            if (!tfs.is_open())
                std::cout << "Error opening tfs";
            dump_to_matlab(msh, dir_name + "/mesh"+ other_info + ".m");

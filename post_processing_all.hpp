...
template<typename MeshType, typename TensorsType>
void
              ...
              const TensorsType & mpt)
{
    ...
    cell_quad_type cell_quadrature(2 * degree);
    face_quad_type face_quadrature(2 * degree);
    ...
    for (auto& cl: msh)
    {
        ...
        for (auto& qp : cell_quadpoints)
            ofs << qp.point().x() << "  "<< qp.point().y() << std::endl;
    }
    ofs << "];"<<std::endl;
    ofs << "tau = [";
    ...
    for (auto& cl: msh)
    {
        vector_type sigma = mpt.at_cell(msh, cl);

        auto cell_quadpoints = cell_quadrature.integrate(msh, cl);

        for (auto& qp : cell_quadpoints)
        {
            auto phi = cell_basis.eval_functions(msh, cl, qp.point(), 0, degree);
            matrix_type vec_phi = make_vectorial_matrix(phi, mesh_type::dimension);

            vector_type  vec = vec_phi * sigma;
            ofs << vec(0) << "  "<< vec(1) << std::endl;
        }
    }
    ofs << "];"<<std::endl;
...
template<typename T>
void
make_fig(const disk::mesh_parameters<T>  &  mp,
        const std::map<std::string, double>& timings)
{
    auto MakeFigs_name = mp.directory + "/MakeFigs" + mp.summary + ".m";

    std::ofstream eeffs(MakeFigs_name);
    if (!eeffs.is_open())
        std::cout << "Error opening file :"<<MakeFigs_name <<std::endl;

        eeffs << "draw_error_contr = true;"<<std::endl;;
        eeffs << "exact = true;"<<std::endl;
        eeffs <<std::endl;
        eeffs << " l=100;  m = 101; n = 102; " <<std::endl;
        eeffs << "if draw_error_contr == false"<<std::endl;;
        eeffs << "  exact = false;"<<std::endl;
        eeffs << "end"<<std::endl;
        eeffs << "times = [];"<<std::endl;


        eeffs << "\% Mesh "<<std::endl;
        eeffs << "draw_mesh  = false;"<<std::endl;
        eeffs << "print_mesh = false;"<<std::endl;

        eeffs << "\% Error "<<std::endl;
        eeffs << "draw_error = true;"<<std::endl;
        eeffs << "print_error= false;"<<std::endl;

        eeffs << "\% Error contributions"<<std::endl;
        eeffs << "draw_error_contributions  = true;"<<std::endl;
        eeffs << "print_error_contributions = false;"<<std::endl;

        eeffs << "\% Solution "<<std::endl;
        eeffs << "draw_solution   = false;"<<std::endl;
        eeffs << "print_solution  = false;"<<std::endl;

        eeffs << "\% Estimator "<<std::endl;
        eeffs << "draw_estimator  = false;"<<std::endl;
        eeffs << "print_estimator = false;"<<std::endl;
        eeffs << "all_meshes = true; "<<std::endl;
        eeffs << std::endl;
        eeffs << "if(all_meshes == true)"<<std::endl;
        eeffs << "    msh = ones(1, "<<mp.num_remesh + 1<<");"<<std::endl;
        eeffs << "else "<<std::endl;
        eeffs << "    msh = zeros(1,"<< mp.num_remesh+ 1<<");"<<std::endl;
        eeffs << "end"<<std::endl;
        eeffs << "%Exact solution errors"<<std::endl;
        eeffs << "if exact == true"<<std::endl;
        eeffs << "  close all;"<<std::endl;
        eeffs << "  col_str = 'velocity error'; exFunction"<< mp.summary <<std::endl;
        eeffs << "  close all;"<<std::endl;
        eeffs << "  col_str = 'sigma error'; exFunction"<< mp.summary <<std::endl;
        eeffs << "end"<<std::endl;        eeffs << "%Exact solution errors"<<std::endl;
        eeffs << std::endl;
        eeffs << "%Residual - steps"<<std::endl;
        eeffs << "close all;"<<std::endl;
        //eeffs << "col_str = 'Total residual'; exFunction"<< mp.summary <<std::endl;
        eeffs << "col_str = 'Residual'; exFunction"<< mp.summary <<std::endl;

        eeffs << "%Residual - contributions"<<std::endl;
        eeffs << " if (draw_error_contributions == true)"<<std::endl;
        eeffs << "      col_str = 'Residual'; row = 1; exFunction"<< mp.summary <<std::endl;
//        eeffs << "      col_str = 'Total residual'; row = 1; exFunction"<< mp.summary <<std::endl;
        eeffs << "      all_meshes = false;"<<std::endl;

        for(size_t k = 0; k <= mp.num_remesh; k++)
        {
            eeffs << "      close all"<<std::endl;
            eeffs << "      msh = zeros(1, "<< mp.num_remesh + 1 <<");"<<std::endl;
            eeffs << "      msh("<< k+1 <<") = 1;"<<std::endl;
            eeffs << "      print_error= false;"<<std::endl;
            //eeffs << "      col_str = 'Total residual'; exFunction"<< mp.summary <<std::endl;
            //eeffs << "      col_str = 'Residual'; exFunction"<< mp.summary <<std::endl;
            eeffs << "      col_str = 'Gu';     exFunction"<< mp.summary << std::endl;
            //eeffs << "      col_str = 'Su';     exFunction"<< mp.summary << std::endl;
            eeffs << "      col_str = 'sigma';  exFunction"<< mp.summary <<std::endl;
            //eeffs << "      col_str = 'varsigma'; exFunction"<< mp.summary <<std::endl;
            eeffs << "      \%legend('Residual', '||G_h^k(u_h^{n+1}- u_h^n||',";
            eeffs << "      '||S_h^k(u_h^{n+1}- u_h^n||', ' ||\\sigma_h^{n+1}- \\sigma_h^n||',";
            eeffs << "      '||\\varsigma_h^{n+1}- \\varsigma_h^n||')"<<std::endl;
            eeffs << "      hline = findobj(gcf, 'type', 'line');"<<std::endl;
            //eeffs << "      set(hline(5),'Color', 'k'); set(hline(5),'Marker','+');  set(hline(5),'MarkerSize',10);"<<std::endl;
            //eeffs << "      set(hline(4),'Color', 'b');"<<std::endl;
            eeffs << "      set(hline(3),'Color', [0.5 0.5 0.5]);"<<std::endl;
            eeffs << "      set(hline(2),'Color', 'g');"<<std::endl;
            eeffs << "      set(hline(1),'Color', 'r');"<<std::endl;
            //eeffs << "      for i =1:5"<<std::endl;
            eeffs << "      for i =1:3"<<std::endl;
            eeffs << "          set(hline(i),'LineWidth',2)"<<std::endl;
            eeffs << "      end"<<std::endl;
            eeffs << "      xlabel('No. of iterations','FontSize',20);"<<std::endl;
            eeffs << "      ylim([1.e-10 1.e0]); xlim([1 1.e5]);" <<std::endl;
            eeffs << "      xticks([1  1.e1 1.e2 1.e3 1.e4 1.e5])" <<std::endl;
            eeffs << "      xticklabels({'10^{0}','10^{1}', '10^{2}','10^{3}', '10^{4}', '10^5'})" <<std::endl;
            eeffs << "      if(draw_error_contributions == true)"<<std::endl;
            eeffs << "          export_fig -dpdf Total_residual_Contr_"<<mp.summary<<"_R"<<k<<".pdf"<<std::endl;
            eeffs << "      end"<<std::endl;
            eeffs << "      mm  = 0.01; x1 = 100;  x2 = 1000; x = x1 : x2;" <<std::endl;
            eeffs << "      loglog(x, mm./x, 'k');" <<std::endl;
            eeffs << "      y = ones(size(x)) * x2;      loglog(y, mm./x, 'k')" <<std::endl;
            eeffs << "      z = (mm/x1) * ones(size(x)); loglog(x, z, 'k')" <<std::endl;
            eeffs << "      t = annotation('textbox'); sz = t.FontSize; t.FontSize = 20;";
            eeffs << "      t.String = '1'; t.Color = 'k';  t.EdgeColor = 'none'"<<std::endl;
            eeffs << "end"<<std::endl;
            eeffs << std::endl;
        }

        eeffs << std::endl;
        eeffs <<" %Total Times"<< std::endl;
        for (auto& t : timings)
            eeffs << " % * " << t.first << ": " << t.second << " seconds." << std::endl;

        eeffs << "total_times = [ " << std::endl;
        for (auto& t : timings)
            eeffs << "  " << t.second <<  std::endl;
        eeffs << " ] " << std::endl;
...
void
colormap_to_matlab(const MeshType& msh, const std::vector<size_t>& marks,
                   const std::string& name)
{
    std::ofstream    mefs(name);
...
0.7 0.7 0.2   %   ocre
    ...
    for(auto& cl : msh )
    {
        ...
        mefs << " coords = [";
        for(auto & p : pts)
            mefs <<" "<< p.x() <<" " << p.y()<<std::endl;
        mefs << " ];"<<std::endl;
        mefs << "hc = fill(coords(:,1), coords(:,2), color_mat(";
        mefs << marks.at(cl_id) + 1<<",:)); "<<std::endl;
        mefs << "set(hc,'EdgeColor','none');"<<std::endl;
        mefs<< "strName = strtrim(cellstr(num2str("<< cl_id<<",'(%d)')));"<<std::endl;
...
template<typename T, size_t DIM, typename Storage, typename MeshParameters,
        typename PlasticData, typename Solution, typename Variable>
void
            ...
            const Solution & solution)
{
    ...
    tensors<mesh<T,DIM,Storage>>   stresses_Th = var.stresses;
    std::vector<dynamic_vector<T>> velocity_Th = var.velocity;
    ...
    std::string  msh_str = tostr(imsh);
    std::string  info =  mp.summary + "_R" + msh_str;
    std::string  info_short =  mp.summary;
    ...
    {
        point_type p1    = point_type({ 0.0, 0.0});
        point_type p2_1  = point_type({1.0, 0.});
        point_type p2_075= point_type({0.75, 0.});
        point_type p2_05 = point_type({0.5, 0.});
        point_type p2_0  = point_type({0.0, 0.});

        pp.plot_over_line(msh, p1, p2_1  , rec_vel_Th, rec_plotline_file + "P1.dat");
        pp.plot_over_line(msh, p1, p2_075, rec_vel_Th, rec_plotline_file + "P075.dat");
        pp.plot_over_line(msh, p1, p2_05 , rec_vel_Th, rec_plotline_file + "P05.dat");
        pp.plot_over_line(msh, p1, p2_0  , rec_vel_Th, rec_plotline_file + "P0.dat");
    }

    save_data(velocity_Th, mp.directory + "/velocities" + info + ".txt");
    save_data(stresses_Th, mp.directory + "/multiplier" + info + ".txt");

    quiver_matlab(msh, mp.directory + "/quiver" + info + ".m", degree, stresses_Th);

    disk::surface_areas<T> surfaces;
    auto marks = marks_yield(msh, var, pst, degree, surfaces);
    colormap_to_matlab(msh, marks, mp.directory + "/criterion" + info + ".m");

    auto exfunc_name = mp.directory + "/exFunction"+ mp.summary +".m";

    std::ofstream effs(exfunc_name);
    if (!effs.is_open())
        std::cout << "Error opening file :"<<exfunc_name <<std::endl;
    effs << "if strcmp (col_str, 'Total residual'); col = 2; end;" <<std::endl;
    effs << "if strcmp (col_str,  'Residual') ;col = 3; end;" <<std::endl;
    effs << "if strcmp (col_str,  'Gu')       ;col = 4; end;" <<std::endl;
    effs << "if strcmp (col_str,  'Su')       ;col = 5; end;" <<std::endl;
    effs << "if strcmp (col_str,  'sigma')    ;col = 6; end;" <<std::endl;
    effs << "if strcmp (col_str,  'varsigma') ;col = 8; end;" <<std::endl;
    effs << "if strcmp (col_str,  'velocity error') ;col = 10; end;" <<std::endl;
    effs << "if strcmp (col_str,  'sigma error')    ;col = 11; end;" <<std::endl;
    ...
    effs << "   set(gca,'fontsize',20); set(gca,'xgrid', 'on', 'ygrid', 'on');"<<std::endl;
    effs << "   if strcmp (col_str, 'Total residual')"<<std::endl;
    effs << "       hline = findobj(gcf, 'type', 'line');"<<std::endl;
    effs << "       set(hline(6),'Color', 'k'); "<<std::endl;
    effs << "       set(hline(5),'Color', 'b'); "<<std::endl;
    effs << "       set(hline(4),'Color', 'r'); "<<std::endl;
    effs << "       set(hline(3),'Color', 'g'); "<<std::endl;
    effs << "       set(hline(2),'Color', 'c'); "<<std::endl;
    effs << "       set(hline(1),'Color', 'm'); "<<std::endl;
    effs << "       for i =1:6"<<std::endl;
    effs << "           set(hline(i),'LineWidth',2)"<<std::endl;
    effs << "       end"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('Residual','FontSize',20);"<<std::endl;
    effs << "       lgd.FontSize = 20;"<<std::endl;
    effs << "       \%title(' Total residual with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       export_fig -dpdf Total_residual"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str, 'Residual')"<<std::endl;
    effs << "       \%title(' Residual with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('Residual_cells','FontSize',20);"<<std::endl;
    effs << "       export_fig -dpdf Residual"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "       end"<<std::endl;
    effs << "       \%title(' |G_h^k(u_h^{n+1}- u_h^n| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "   if strcmp (col_str, 'Gu')"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|G_h^k(u_h^{n+1}- u_h^n|','FontSize',20);"<<std::endl;
    effs << "       export_fig -dpdf GUerror"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str, 'Su')"<<std::endl;
    effs << "       \%title(' |S_h^k(u_h^{n+1}- u_h^n)| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|S_h^k(u_h^{n+1}- u_h^n|','FontSize',20);"<<std::endl;
    effs << "       export_fig -dpdf SUerror"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str, 'sigma')"<<std::endl;
    effs << "       \%title(' |\\sigma_h^{n+1}- \\sigma_h^n| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|\\sigma_h^{n+1}- \\sigma_h^n|','FontSize',20);"<<std::endl;
    effs << "       export_fig -dpdf Sigma"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str, 'varsigma')"<<std::endl;
    effs << "       \%title(' |\\varsigma_h^{n+1}- \\varsigma_h^n)| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|\\varsigma_h^{n+1}- \\varsigma_h^n|','FontSize',20);"<<std::endl;
    effs << "     export_fig -dpdf Varsigma"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str,  'velocity error')"<<std::endl;
    effs << "       \%title(' |u_h^{n+1}- I_hu)| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|u_h^{n+1}- I_hu|','FontSize',20);"<<std::endl;
    effs << "       hline = findobj(gcf, 'type', 'line');"<<std::endl;
    effs << "       set(hline(3),'Color', 'k'); "<<std::endl;
    effs << "       set(hline(2),'Color', 'b'); "<<std::endl;
    effs << "       set(hline(1),'Color', 'r'); "<<std::endl;
    effs << "       for i =1:3"<<std::endl;
    effs << "           set(hline(i),'LineWidth',2)"<<std::endl;
    effs << "       end"<<std::endl;
    effs << "       lgd.FontSize = 20;"<<std::endl;
    effs << "       hline = findobj(gcf, 'type', 'line');"<<std::endl;
    ...
    effs << "       set(hline(2),'Color', 'b'); "<<std::endl;
    effs << "       set(hline(1),'Color', 'r'); "<<std::endl;
    effs << "       export_fig -dpdf Uerror"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "   if strcmp (col_str,  'sigma error')"<<std::endl;
    effs << "       \%title(' |\\sigma_h^{n+1}- I_h\\sigma)| with k = "<< degree<<"; Bi = "<<pst.Bn<<"');"<<std::endl;
    effs << "       xlabel('No.of iterations','FontSize',20); ylabel('|\\sigma_h^{n+1}- I_h\\sigma|','FontSize',20);"<<std::endl;
    effs << "       hline = findobj(gcf, 'type', 'line');"<<std::endl;
    effs << "       \%      set(hline(3),'Color', 'k'); "<<std::endl;
    effs << "       \%      set(hline(2),'Color', 'b'); "<<std::endl;
    effs << "       \%      set(hline(1),'Color', 'r'); "<<std::endl;
    effs << "       set(hline(2),'Color', 'k'); "<<std::endl;
    effs << "       set(hline(1),'Color', 'b'); "<<std::endl;
    effs << "       for i =1:2"<<std::endl;
    effs << "           set(hline(i),'LineWidth',2)"<<std::endl;
    effs << "       end"<<std::endl;
    effs << "       lgd.FontSize = 20;"<<std::endl;
    effs << "       legend('Location', 'southeast')"<<std::endl;
    effs << "       export_fig -dpdf Sig_error"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    effs << "   end"<<std::endl;
    effs << "end"<<std::endl;
    effs.close();
        ...
        const std::map<std::string, double>& timings)
{
    ...
    if(imsh > 0)
    {
        ...
        exfs<< "        set(gca,'box','on'); set(gcf,'color','w');set(gca,'fontsize',20);"<<std::endl;
        exfs<< "        circle(0,0,"<<pst.Bn<<",'w')"<<std::endl;
        exfs<< "        export_fig -dpdf estimator_tot"<<mp.summary<<"_RC"<<imsh<<".pdf"<<std::endl;
        exfs<< "    end;"<<std::endl;
        exfs<< "    figure;estimator_res"<<mp.summary<<"_RC"<<imsh<<";";
        exfs<< "    if print_estimator == true";
        exfs<< "        set(gca,'box','on'); set(gcf,'color','w');set(gca,'fontsize',20);"<<std::endl;
        exfs<< "        circle(0,0,"<<pst.Bn<<",'w')"<<std::endl;
        exfs<< "        export_fig -dpdf estimator_res"<<mp.summary<<"_RC"<<imsh<<".pdf"<<std::endl;
        exfs<< "    end;"<<std::endl;
        exfs<< "    figure;estimator_str"<<mp.summary<<"_RC"<<imsh<<";";
        exfs<< "    if print_estimator == true";
        exfs<< "        set(gca,'box','on'); set(gcf,'color','w');set(gca,'fontsize',20);"<<std::endl;
        exfs<< "        circle(0,0,"<<pst.Bn<<",'w')"<<std::endl;
        exfs<< "        export_fig -dpdf estimator_str"<<mp.summary<<"_RC"<<imsh<<".pdf"<<std::endl;
        exfs<< "    end;"<<std::endl;
        exfs<< "end"<<std::endl;
    }
    ...
    exfs<< "    figure(l); hold on; "<< std::endl;
    exfs<< "    loglog(e"<<imsh<<"(:,row),e"<<imsh<<"(:,col), 'DisplayName', 'step "<<imsh<<"' , 'color', rand(1,3));"<<std::endl;
    exfs<< "    set(gca,'box','on'); set(gcf,'color','w');"<<std::endl;
    exfs<< "    set(gca,'fontsize',20); set(gca,'xgrid', 'on', 'ygrid', 'on');"<<std::endl;
    ...
    exfs<< "        export_fig -dpdf mesh_B"<<tostr(10*pst.Bn)<<mp.summary<<"_R" <<imsh<<".pdf"<<std::endl;
    exfs <<"    end"<<std::endl;
    exfs <<"end"<<std::endl;
    exfs<< std::endl;
    ...
    exfs<< "        set(gca,'box','on'); set(gcf,'color','w');set(gca,'fontsize',20);"<<std::endl;
    exfs<< "        export_fig -dpdf plotoline_sol_"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    exfs<< "    end;"<<std::endl;
    exfs<< std::endl;
    ...
    exfs<< "    if print_solution == true";
    exfs<< "        set(gca,'box','on'); set(gcf,'color','w');set(gca,'fontsize',20);"<<std::endl;
    exfs<< "        export_fig -dpdf plotoline_grad_"<<mp.summary<<"_R"<<imsh<<".pdf"<<std::endl;
    exfs<< "    end;"<<std::endl;
    ...
    for (auto& t : timings)
        exfs << "% * " << t.first << ": " << t.second << " seconds." << std::endl;

    exfs << " times = [ times; " << std::endl;
    for (auto& t : timings)
        exfs << "  " << t.second <<  std::endl;
    exfs << " ] " << std::endl;

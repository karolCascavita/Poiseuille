...
template<typename MeshType, typename T,
                                typename CellBasisType, typename FaceBasisType>
void
jump_to_matlab( const MeshType& msh,
                const std::vector<tensors<MeshType>>& tsr_vec,
                const std::vector<dynamic_vector<T>>& Uh_Th,
                ...
                const size_t imsh)
{
    std::cout << "INSIDE JUMPS DISCRETE" << std::endl;
    typedef MeshType          mesh_type;
    typedef typename mesh_type::cell    cell_type;
    typedef typename mesh_type::face    face_type;
    ...
    std::string jump_file = mp.directory + "/jumps" + mp.summary + "_R" + tostr(imsh)+".dat";

    std::ofstream jfs(jump_file);

    if (!jfs.is_open())
        std::cout << "Error opening file :"<<jump_file <<std::endl;

    auto face_owners_vec = faces_owners(msh);
    size_t i = 0;
    for(auto fitor = msh.faces_begin(); fitor != msh.faces_end(); fitor++)
    {
        auto face = *fitor;
        //std::cout << "FACE : "<< msh.lookup(face)<< "  ; i : "<< i << std::endl;
        if(!msh.is_boundary(face))
        {
            auto cells = face_owners_vec.at(i);

            auto cell1_id = cells.first;
            auto cell2_id = cells.second;

            auto cell1 = *std::next(msh.cells_begin(), cell1_id);
            auto cell2 = *std::next(msh.cells_begin(), cell2_id);

            vector_type c1_uh_TF   =  Uh_Th.at(cell1_id);
            vector_type c2_uh_TF   =  Uh_Th.at(cell2_id);

            matrix_type c1_rec_oper =  grad_global.at(cell1_id);
            matrix_type c2_rec_oper =  grad_global.at(cell2_id);

            vector_type c1_ruh  =  c1_rec_oper * c1_uh_TF;
            vector_type c2_ruh  =  c2_rec_oper * c2_uh_TF;

            auto c1_gamma  =  tsr_vec.at(cell1_id).gamma;
            auto c2_gamma  =  tsr_vec.at(cell2_id).gamma;
            auto c1_sigma  =  tsr_vec.at(cell1_id).sigma;
            auto c2_sigma  =  tsr_vec.at(cell2_id).sigma;

            auto c1_nFT    =  normal_factor(msh, cell1, face);
            auto c2_nFT    =  normal_factor(msh, cell2, face);

            //std::cout << "c1_nFT : "<< c1_nFT << "   ; c2_nFT : "<< c2_nFT << std::endl;
            auto f_normal  =  normal_face(msh, face);

            auto fqs = face_quadrature.integrate(msh, face);

            auto c1_face_pos  = face_position(msh, cell1, face);
            auto c2_face_pos  = face_position(msh, cell2, face);

            auto cq1_range = c1_sigma.qsr.face_range(c1_face_pos);
            auto cq2_range = c2_sigma.qsr.face_range(c2_face_pos);;

            auto c1_qp_pos = cq1_range.min();
            auto c2_qp_pos = cq2_range.min();

            matrix_type c1_tau  = c1_sigma.face_quad_pts
                                        - pst.alpha * c1_gamma.face_quad_pts;
            matrix_type c2_tau  = c2_sigma.face_quad_pts
                                        - pst.alpha * c2_gamma.face_quad_pts;

            //std::cout << " * c1 : "<< cell1_id << "   ; c2_: "<< cell2_id << std::endl;
            //std::cout << " * fpos1  : "<< c1_face_pos << "   ; fpos2  : "<< c2_face_pos << std::endl;
            //std::cout << " * cqpos1 : "<< c1_qp_pos   << "   ; cqpos2 : "<< c2_qp_pos << std::endl;

            for (auto& qp : fqs)
            {
                auto c1_dphi = cell_basis.eval_gradients(msh, cell1, qp.point());
                auto c2_dphi = cell_basis.eval_gradients(msh, cell2, qp.point());

                matrix_type c1_dphi_matrix = make_gradient_matrix(c1_dphi);
                matrix_type c1_dphi_taken  = take(c1_dphi_matrix, row_range, col_range);
                vector_type c1_dphi_ruh    = c1_dphi_taken * c1_ruh;

                matrix_type c2_dphi_matrix = make_gradient_matrix(c2_dphi);
                matrix_type c2_dphi_taken  = take(c2_dphi_matrix, row_range, col_range);
                vector_type c2_dphi_ruh    = c2_dphi_taken * c2_ruh;

                //estimator only with plasticity
                vector_type c1_str  = c1_tau.col(c1_qp_pos) + pst.alpha * c1_dphi_ruh;
                vector_type c2_str  = c2_tau.col(c2_qp_pos) + pst.alpha * c2_dphi_ruh;

                vector_type jump_du = (c1_dphi_ruh * c1_nFT) + (c2_dphi_ruh * c2_nFT);
                vector_type jump_st = c1_tau.col(c1_qp_pos) * c1_nFT
                                                + c2_tau.col(c2_qp_pos) * c2_nFT;
                vector_type jump_tot = (c1_str * c1_nFT)  + (c2_str * c2_nFT);

                auto du_jvalue  = pst.alpha * make_prod( jump_du , f_normal);
                auto st_jvalue  = make_prod( jump_st , f_normal);
                auto tot_jvalue = make_prod( jump_tot, f_normal);

                jfs <<" "<< qp.point().x()<<" "<< qp.point().y()<< " ";
                jfs << du_jvalue<<"  "<< st_jvalue <<"  "<< tot_jvalue<<std::endl;
            }
        }
        else
        {
            ...
            //std::cout << " * cells: "<< cells.first<< " "<< cells.second << std::endl;
            auto cell1 = *std::next(msh.cells_begin(), cell1_id);
            ...
            for (auto& qp : fqs)
            {
                ...
                vector_type jump = (c1_str * c1_nFT);
                auto jp_value    =  make_prod( jump , f_normal);

                vector_type jump_du  = (c1_dphi_ruh * c1_nFT);
                vector_type jump_st  =  c1_tau.col(c1_qp_pos) * c1_nFT;
                vector_type jump_tot = (c1_str * c1_nFT);
...
template<typename MeshType, typename T,
        typename CellBasisType, typename FaceBasisType, typename Solution>
void
jump_to_matlab( const MeshType& msh,
                const Solution& solution,
                const std::vector<dynamic_vector<T>>& Uh_Th,
                ...
                const size_t imsh)
{
    std::cout << "INSIDE JUMPS DIFFUSION" << std::endl;
    ...
    for(auto fitor = msh.faces_begin(); fitor != msh.faces_end(); fitor++)
    {
        ...
        if(!msh.is_boundary(face))
        {
            ...
            //std::cout << " * fpos1  : "<< c1_face_pos << "   ; fpos2  : "<< c2_face_pos << std::endl;
            //std::cout << " * cqpos1 : "<< c1_qp_pos   << "   ; cqpos2 : "<< c2_qp_pos << std::endl;
            auto number_c1 = set_cell_number(msh, cell1);
            auto number_c2 = set_cell_number(msh, cell2);
            ...
            for (auto& qp : fqs)
            {
                ...
                //estimator only with plasticity
                vector_type c1_tau  =  solution.st( qp.point(), number_c1);
                vector_type c2_tau  =  solution.st( qp.point(), number_c2);

                vector_type c1_str  =  c1_tau + pst.alpha * c1_dphi_ruh;
                vector_type c2_str  =  c2_tau + pst.alpha * c2_dphi_ruh;

                vector_type jump_du = (c1_dphi_ruh * c1_nFT) + (c2_dphi_ruh * c2_nFT);
                vector_type jump_st =  c1_tau * c1_nFT + c2_tau * c2_nFT;
        ...
        else
        {
            ...
            for (auto& qp : fqs)
            {
                ...
                vector_type jump_du  = (c1_dphi_ruh * c1_nFT);
                vector_type jump_st  =  c1_tau * c1_nFT;
                vector_type jump_tot = (c1_str * c1_nFT);

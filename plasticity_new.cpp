...
{
        variable(){}
        std::vector<dynamic_vector<T>>  Uh_vector;
        std::vector<disk::tensors<T>>   tensor_vector;

        variable& operator=(const variable& other) {
            Uh_vector     = other.Uh_vector;
            tensor_vector = other.tensor_vector;
            return *this;
        }
        ...
        {
            Uh_vector     = solution_zero_vector(msh, degree);
            tensor_vector = disk::tensor_zero_vector(msh, degree);
        }
...
public:
    ...
    plasticity_problem(variable<T> var,
                       const mesh_type& msh,
                       const size_t &     degree,
    ...
    : m_degree(degree), m_max_iters(max_iters)
    {
        ...
        m_quad_degree = var.tensor_vector.at(0).quad_degree;

        std::cout << "m_quad_degree "<< m_quad_degree << std::endl;
        std::cout << "size tsr    "<<var.tensor_vector.at(0).xi_norm.rows();
        std::cout << " x" << var.tensor_vector.at(0).xi_norm.cols()<< std::endl;
        ...
        else
        {
            if(!mp.mark_all)
            {
                ...
                auto any = sbm.template marker<cell_basis_type, face_basis_type, Solution>
                        (old_msh, var.tensor_vector, pst, var.Uh_vector, m_degree,
                        ...
                        const size_t imsh)
    {
        ...
        if(imsh > 0 && mp.recycle)
        {
            ...
            var.Uh_vector = disk::proj_rec_solution< mesh_type, T,
                            cell_basis_type, cell_quadrature_type,
                                face_basis_type, face_quadrature_type,
                                                            point_type>
            (new_msh, old_msh, grad_global, old_var.Uh_vector, ancestors_vec, m_degree);

            var.tensor_vector = sigma_interpolation(new_msh, old_msh,
                                            old_var.tensor_vector, ancestors_vec,
                                                        cells_marks, m_degree);
            std::vector<matrix_type>   sigma;
            auto quad_degree = var.tensor_vector.at(0).quad_degree;
            std::cout << "SAVING SIGMA_INTER_R" << std::endl;
            auto filename = mp.directory + "/SIGMA_INTER_R" + tostr(imsh)+ ".m";
            get_from_tensor(sigma, var.tensor_vector, "sigma");
                    ...
                    const size_t imsh)
    {
        ...
        else
        {
            ...
            throw std::logic_error("It shouldn't be coming here. Define procedure for this case or set call_mesher as true.");
            return std::make_pair(false, old_msh);
        }
    }
                            ...
                            const size_t imsh)
    {
        ...
        else
        {
            ...
            disk::load_data(old_var.Uh_vector, mp, "/Uh","R", imsh - 1);
            disk::load_data(old_var.tensor_vector, mp, "R",imsh);
            assert( old_var.Uh_vector.size()  == old_msh.cells_size());
            assert( old_var.tensor_vector.size()== old_msh.cells_size());
                     ...
                     const T& alpha)
    {
        ...
        for(auto& cl : msh)
        {
            ...
            matrix_type    new_sigma   = new_var.tensor_vector.at(cl_id).siglam;
            matrix_type    old_sigma   = old_var.tensor_vector.at(cl_id).siglam;
            matrix_type    sigma_diff  = new_sigma - old_sigma;

            matrix_type    rec_oper  = grad_global.at(cl_id);
            vector_type    uh_diff   = new_var.Uh_vector.at(cl_id)
                                        - old_var.Uh_vector.at(cl_id);
             ...
             std::ofstream      & tfs)
    {
        ...
        if(is_finished)
        {
            ...
            tfs<<m_degree<<"  " << pst.alpha <<"  " <<h_max <<"   "<< error.dof<<std::endl;
    ...
    template<typename Solution, typename Variable>
    auto
                       ...
                       const Solution& solution)
    {
        ...
        for (auto& qp : qps_pst)
        {
            vector_type dphi_ruh  = disk::nabla_ruh_at_point(qp.point(), msh, cl,
                                                            cb, ruh, m_degree);
            matrix_type gamma_mat = var.tensor_vector.at(cell_id).gamma;
            vector_type gamma     = gamma_mat.col(col);
                    ...
                    const scalar_type   &  on_pst)
    {
        ...
        for(auto& cl: msh)
        {
            ...
            vector_type pst_rhs;
            vector_type u_local = var.Uh_vector.at(cell_id);
            auto tsr = var.tensor_vector.at(cell_id);
            ...
            //WK: if diffusion changes with domain, it would be mandatory to
                      ...
                      const scalar_type &  on_pst)
    {
        ...
        for (auto& cl : msh)
        {
            ...
            if(mp.diff)  //Only for diffusion test
                ...
                pst_rhs  = plasticity.compute(msh, cl, rec_oper, u_local, pst, var.tensor_vector.at(cell_id));

            vector_type x = statcond_pst.recover(msh, cl, loc_mat, cell_rhs, xFs,
                                                              on_pst * pst_rhs);
            var.Uh_vector.at(cell_id)  = x;
            ...
            compute_cell_error(error, msh, cl, var, x, ruh, solution);
        }
                    ...
                    const size_t imsh)
    {
        ...
        if(imsh == 1)
            auto er = disk::postprocess(msh, solution, var.tensor_vector,
                                    var.Uh_vector, pst, mp, m_degree, imsh + 80);
        ...
        for(size_t iter = 0; iter < m_max_iters ; iter++)
        {
            ...
            size_t on_pst = 1;
            if(iter == 0)
                if( (mp.recycle && imsh == 0) || !mp.recycle )
                    on_pst =  0;
            ...
            vector_type  X = make_rhs_vector(msh, var, biforms, pst, solution, mp,
                                                                  kappa, on_pst);
            auto error = recover_solution(var, msh, X, biforms, pst, solution, mp,
                                                                  kappa, on_pst);
        ...
        auto er = disk::postprocess(msh, solution,  var.tensor_vector, var.Uh_vector,
                                    pst, mp, m_degree, imsh);
        disk::execute(exfs, mp, imsh);
                        ...
                        const size_t imsh)
    {
        ...
        for(size_t iter = 0; iter < m_max_iters ; iter++)
        {
            ...
            ifs<<tostr(iter) <<"  "<<error.gamma<<" "<<error.dof<<" "<<solver_conv<<" "<< residue<<std::endl;
                ...
                bool  & use_ADMM)
{
    ...
    int  method_name,hanging_on,refine_on, num_adapts, temp_recycle, temp_ADMM, temp_reset_off;
...
int main (int argc, char** argv )
{
    ...
    if (std::regex_match(filename, std::regex(".*\\.typ1$") ))
    {
        ...
        std:: string root = "circular/Meeting5-Correction/Method1/";

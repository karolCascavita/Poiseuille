...
struct variable
{
        ...
        variable(){}
        std::vector<dynamic_vector<scalar_type>>   Uh_vector;
        std::vector<disk::tensors<MeshType>>   tensor_vector;
...
public:
    ...
    : m_degree(degree), m_max_iters(max_iters)
    {
        ...
        m_quad_degree = var.tensor_vector.at(0).sigma.m_quad_degree;
                        ...
                        const size_t imsh)
    {
        ...
        if(imsh > 0 && mp.recycle)
        {
            ...
            auto quad_degree = var.tensor_vector.at(0).sigma.m_quad_degree;
            for(auto& cl: new_msh)
            {
                size_t id    = cl.get_id();
                sigma.at(id) = var.tensor_vector.at(id).sigma;
            }
            std::cout << "SAVING SIGMA_INTER_R" << std::endl;
            auto filename = mp.directory + "/SIGMA_INTER_R" + tostr(imsh)+ ".m";
            disk::get_from_tensor(sigma, var.tensor_vector, "sigma");
                            ...
                            const size_t imsh)
    {
        ...
        else
        {
            ...
            disk::load_data(old_var.tensor_vector, old_msh, mp, "R",imsh);
                     ...
                     const T& alpha)
    {
        ...
        for(auto& cl : msh)
        {
            ...
            auto new_all_sigma  = new_var.tensor_vector.at(cl_id).sigma;
            auto old_all_sigma  = old_var.tensor_vector.at(cl_id).sigma;
             ...
             std::ofstream      & tfs)
    {
        ...
        for(auto& cl: msh)
        {
            ...
            tensor_DOFs += num_fqps * num_faces;
        }
                       ...
                       const Solution& solution)
    {
        ...
        for (auto& qp : qps_pst)
        {
            vector_type dphi_ruh  = disk::nabla_ruh_at_point(qp.point(), msh, cl,
                                                            cb, ruh, m_degree);
            auto all_gamma    = var.tensor_vector.at(cell_id).gamma;
            auto all_sigma    = var.tensor_vector.at(cell_id).sigma;
            auto all_sigma_old= old_var.tensor_vector.at(cell_id).sigma;
                      ...
                      const scalar_type &  on_pst)
    {
        ...
        for (auto& cl : msh)
        {
            ...
            compute_cell_error(error, msh, cl, var, old_var, x, ruh, solution);
        }
                    ...
                    const size_t imsh)
    {
        ...
        for(size_t iter = 0; iter < m_max_iters ; iter++)
        {
            ...
            auto error = recover_solution(var, old_var, msh, X, biforms, pst, solution, mp,
                                                                  kappa, on_pst);
                        ...
                        const size_t imsh)
    {
        ...
        for(size_t iter = 0; iter < m_max_iters ; iter++)
        {
            ...
            ifs<<tostr(iter) <<"  "<<error.gamma<<" "<<error.sigma<<" ";
            ifs<<error.dof<<" "<<solver_conv;
            ifs<<"  "<< residue<< " "<< pst.alpha<<"  "<< pst.beta  <<std::endl;
                    ...
                    const size_t imsh)
    {
        ...
        for(size_t iter = 0; iter < m_max_iters ; iter++)
        {
            ...
            for (auto& cl : msh)
            {
                 ...
                 vector_type pst_rhs  = plasticity.compute(msh, cl, rec_oper, u_local, pst, var.tensor_vector.at(cell_id));

                var.Uh_vector.at(cell_id)  = u_local;
                vector_type ruh = rec_oper * u_local;
                compute_cell_error(error, msh, cl, var, old_var, u_local, ruh, solution);
            }
            error.sigma    = std::sqrt(error.sigma);
            error.gamma    = std::sqrt(error.gamma);
            error.dof      = std::sqrt(error.dof);

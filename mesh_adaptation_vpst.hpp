...
struct surface_areas
{
    ...
    T liquid;
    T tol;
    friend std::ostream& operator <<(std::ostream& os, const surface_areas<T>& sa)
    {
        os << "Surfaces liquid-solid: "<<std::endl;
        os << "* Liquid  : "<< sa.liquid << std::endl;
        os << "* Tolerance : "<< sa.tol<< std::endl;
        os << "* Solid   : "<< sa.rigid << std::endl;
        os << "*** rigid : "<< sa.rigid_zero<< std::endl;
        os << "*** mov   : "<< sa.rigid_mov<< std::endl;
        return os;
    }
};
...
auto
marks_yield(const MeshType& msh, const Variable& var,
            const disk::plasticity_data<typename MeshType::scalar_type>& pst,
            const size_t degree,
            surface_areas<typename MeshType::scalar_type> & surfaces)
{
    typedef MeshType                     mesh_type;
    ...
    for(auto & cl : msh)
    {
        ...
        {
            marks.at(cell_id) = 1;
            surfaces.liquid += measure(msh, cl);
        }
        else
        {
            surfaces.rigid+= measure(msh, cl);
            vector_type v = var.velocity.at(cell_id);
            //Warning this position is only for k = 0. We take the only vel in the cell
            if( std::abs(v(0)) < surfaces.tol )
                surfaces.rigid_zero+= measure(msh, cl);
            else
                surfaces.rigid_mov += measure(msh, cl);
        }
...
public:
    ...
    std::vector<std::array<size_t,3>> level_ancestor;
    ...
    std::vector<std::array<size_t,3>>        l_triangles;
    std::vector<std::array<size_t,3>>        l_quadrangles;
    std::vector<std::array<size_t,3>>        l_pentagons;
    std::vector<std::array<size_t,3>>        l_hexagons;
    std::vector<std::array<size_t,3>>        l_heptagons;
    std::vector<std::array<size_t,3>>        l_octagons;
    std::vector<std::array<size_t,3>>        l_enneagons;
    std::vector<std::array<size_t,3>>        l_decagons;
    std::vector<std::array<size_t,3>>        l_hendecagons;
    std::vector<std::array<size_t,3>>        l_dodecagons;
    std::vector<std::array<size_t,3>>        l_triadecagons;
    std::vector<std::array<size_t,3>>        l_tesseradecagons;
    std::vector<std::array<size_t,3>>        l_pentadecagons;
    std::vector<std::array<size_t,3>>        l_16gons;
    std::vector<std::array<size_t,3>>        l_17gons;
    std::vector<std::array<size_t,3>>        l_18gons;
    std::vector<std::array<size_t,3>>        l_19gons;
    std::vector<std::array<size_t,3>>        l_20gons;
    std::vector<std::array<size_t,3>>        l_21gons;
    std::vector<std::array<size_t,3>>        l_22gons;
    std::vector<std::array<size_t,3>>        l_23gons;
    std::vector<std::array<size_t,3>>        l_24gons;
    std::vector<std::array<size_t,3>>        l_25gons;
                        ...
                        imsh(adaptive_step)
    {
        ...
        binning_marks = std::vector<size_t>(msh.cells_size(),0);
        levels = levels_vec;
    ...
    check_levels(const mesh_type& msh, const cell_type & cl)
    {
        ...
        for(auto& fc : fcs)
        {
            ...
            if(cl_id == 0)
                std::cout <<"* Cell : "<< cl_id <<  ": Level: "<<cl_level << std::endl;
            if(!(msh.is_boundary(fc)))
            {
                size_t ngh_id    = msh.neighbor_id(cl, fc);
                auto   ngh_level = levels.at(ngh_id);
                auto diff =  std::abs(int(ngh_level - cl_level));
                if(diff > 1000)
                    std::cout << "* Level(10000) found. Cell : "<< cl_id << " ; Ngh : "<<ngh_id << std::endl;
                if( diff  >= 2 && diff < 1000)// for cell coming from binning
                {
    ...
    bool
    borrar_marker(const mesh_type& msh,
                const size_t  imsh)
    {
        bool do_refinement = false;
    ...
    auto
    temporal_marker_yield(const mesh_type& msh)
    {
        typedef dynamic_vector<scalar_type>                 vector_type;
        typedef dynamic_matrix<scalar_type>                 matrix_type;

        std::cout << "INSIDE_TEMPORAL_YIELD_MARKER " << std::endl;
        bool do_refinement(false);

        auto initial_marks   = std::vector<size_t>(msh.cells_size(), 0);
        for(auto& cl : msh)
        {
            auto id = msh.lookup(cl);
            if( !(id >= 50 &&  id <= 70) && !(id >= 83 && id <= 94) && !(id >= 99 && id <= 101)&& id != 16 && id != 17 && id != 38&& id != 39&& id != 73&& id != 74)
                initial_marks.at(id) = 1;
        }

        std::cout << "directory : "<< mp.directory  << std::endl;
        ...
        auto DIM = mesh_type::dimension;
        cells_marks   = std::vector<size_t>(msh.cells_size());
        binning_marks = std::vector<size_t>(msh.cells_size());
        ...
        dump_to_matlab(msh, mp.directory + "/agglomeration_marks" +  info + ".m", binning_marks);
    ...
    auto
    test_marker_yield(const mesh_type& msh, const Variable& var,
                    const disk::plasticity_data<T>& pst, const size_t degree)
    {
        ...
        surface_areas<scalar_type> surfaces;
        auto initial_marks = marks_yield(msh, var, pst, degree, surfaces);
            ...
            const std::vector<matrix_type>& grad_global)
    {
        ...
        switch(mp.marker_name)
        {
            ...
            case 3:
                return ret = test_marker_yield(msh, var, pst, m_degree);
   ...
   put_n_gon_with_hanging_node(mesh_type & msh, const cell_type& cl)
   {
    ...
    refine_reference_triangle(const n_gon<3> & t , const std::array<int, 3> & bar_ids,
                             const size_t& cl_level, const size_t& cl_id, const size_t& bin_mark)
    {
        n_gon<3> t0,t1,t2,t3;
                                ...
                                const size_t& cl_level, const size_t& bin_mark)
    {
        ...
        auto t  = put_n_gon<3>(msh, cl);
        refine_reference_triangle( t,  bar_ids, cl_level, cl.get_id(), bin_mark);
    }

    void
    refine_other_4tri(mesh_type& msh, const cell_type& cl, const size_t& cl_level,
                                                        const size_t& bin_mark)
    {
        ...
        for(size_t i = 0; i < num_fcs; i++)
        {
            ...
            refine_reference_triangle(nt , tbar_ids, cl_level, cl.get_id(), bin_mark);
        }
    }
    void
                                                        ...
                                                        const size_t& bin_mark)
    {
        ...
        for(size_t i = 0; i < num_fcs; i++)
        {
            ...
            std::array<size_t, 3> tri_levels = {cl_level, cl.get_id(), bin_mark};
            l_triangles.push_back(tri_levels);
        }
    }
                            ...
                            const size_t& bin_mark)
    {
        ...
        for(auto & v:vertices)
        {
            ...
            else
            {
                ...
                std::array<size_t, 3> tri_levels = {cl_level, cl.get_id(), bin_mark};
                l_quadrangles.push_back(tri_levels);
            }
            i++;
        }
    }
                            ...
                            const size_t& bin_mark)
    {
        ...
        for(size_t j = 0; j < 3; j++)
        {
            ...
            if(new_num_fcs == 5)
            {
                ...
                std::array<size_t, 3> tri_levels = {cl_level, cl.get_id(), bin_mark};
                l_pentagons.push_back(tri_levels);
            }
            if(new_num_fcs == 6)
            {
                ...
                l_hexagons.push_back(tri_levels );
            }
            if(new_num_fcs == 7)
            {
                ...
                std::array<size_t, 3> tri_levels = {cl_level, cl.get_id(), bin_mark};
                l_heptagons.push_back(tri_levels );
            }
            ...
            if(new_num_fcs == 8)
            {
                ...
                store(nt,m_octagons);
                std::array<size_t, 3> tri_levels = {cl_level, cl.get_id(), bin_mark};
                l_octagons.push_back(tri_levels );
            }
            ...
            if(new_num_fcs == 9)
            {
                ...
                l_enneagons.push_back(tri_levels );
            }
            ...
            if(new_num_fcs == 10)
            {
                ...
                l_decagons.push_back(tri_levels );
            }
            if(new_num_fcs == 11)
            {
                ...
                l_hendecagons.push_back(tri_levels );
            }
            ...
            if(new_num_fcs == 12)
            {
                ...
                l_dodecagons.push_back(tri_levels );
            }
            if(new_num_fcs == 13)
            {
                ...
                l_triadecagons.push_back(tri_levels );
            }
            if(new_num_fcs == 14)
            {
                ...
                l_tesseradecagons.push_back(tri_levels );
            }
            if(new_num_fcs == 15)
            {
                ...
                l_pentadecagons.push_back(tri_levels );
            }
            if(new_num_fcs == 16)
            {
                                     ...
                                     const size_t& cl_id)
    {
        ...
        auto bin_mark = binning_marks.at(cid);
            ...
            if(msh.is_special_cell(cl))
            {
                ...
                switch (vertices_pos.size())
                {
                    ...
                    case 3:
                        ...
                        refine_special_triangle(msh, cl, vertices_pos, cl_level, bin_mark);
                        break;
                    //case 4:
                    //    refine_quadrangle();
                    //    break;
                    default:
                        ...
                        refine_special_other(msh, cl, vertices_pos, cl_level, bin_mark); //WK: try to do the same for quadrangles
                        break;
                }
            }
            else
            {
                switch (num_fcs)
                {
                    ...
                    case 3:
                        ...
                        refine_triangle(msh, cl, cl_level, bin_mark);
                        break;
                    //case 4:
                    //    refine_quadrangle();
                    //    break;
                    default:
                        ...
                        refine_other(msh ,cl, cl_level, bin_mark);
                        break;
                }
    ...
    save_levels_info(const LoaderType& loader,
                     const std::vector<std::array<size_t,3>>& ancestor_info)
    {
        ...
        for(size_t i = 0; i < ancestor_info.size(); i++)
        {
            ...
            auto tri_inf = *std::next(ancestor_info.begin(), idx);
            auto ancestor_level = tri_inf[0];
            auto ancestor_id    = tri_inf[1];

            lfs << idx << " "<< ancestor_level << " "<<ancestor_id<<  tri_inf[2]<< std::endl;
        }
        lfs.close();
    }
    ...
    refine( const mesh_type & old_msh,
            const size_t    & degree)
    {
        std::cout << "INSIDE REFINE" << std::endl;
        ...
        for(auto& cl : msh)
        {
            auto cl_id   = cl.get_id();
            auto cl_mark = cells_marks.at(cl_id);
            auto bin_mark = binning_marks.at(cl_id);

            std::cout << "1. cell_mark ("<< cl_id <<") ="<< cl_mark << std::endl;

            if(cl_mark > 0) // 10000 is the flag (level) for cells coming from binning
            {
                if(mp.hanging_nodes)
            ...
            else
            {
                ...
                std::cout << " * num_fcs : "<< num_fcs << std::endl;
                ...
                else if(num_fcs == 4)//    case 4:
                {
                    std::cout << " * case 4" << std::endl;
                    store(put_n_gon<4>(msh,cl), m_quadrangles);
                    std::cout << " * crashing 1" << std::endl;
                    std::array<size_t, 3> ngon4_levels = {cl_level, cl.get_id(), bin_mark};
                    std::cout << " ** add quadrangle (cell_mark = 0)" << std::endl;
                    l_quadrangles.push_back(ngon4_levels );
                    //break;
                }
                //case 5:
                else if(num_fcs == 5)
                {
                        ...
                        std::array<size_t, 3> ngon5_levels = {cl_level, cl.get_id(), bin_mark};
                        l_pentagons.push_back(ngon5_levels );
                        //break;
                }
                else if(num_fcs == 6){//case 6:
                        store(put_n_gon<6>(msh,cl), m_hexagons);
                        std::array<size_t, 3> ngon6_levels = {cl_level, cl.get_id(), bin_mark};
                        l_hexagons.push_back(ngon6_levels );
                        //break;
                    }
                else if(num_fcs == 7){//case 7:
                        store(put_n_gon<7>(msh,cl), m_heptagons);
                        std::array<size_t, 3> ngon7_levels = {cl_level, cl.get_id(), bin_mark};
                        l_heptagons.push_back(ngon7_levels );
                        //break;
                    }
                else if(num_fcs == 8){//case 8:
                        store(put_n_gon<8>(msh,cl), m_octagons);
                        std::array<size_t, 3> ngon8_levels = {cl_level, cl.get_id(), bin_mark};
                        l_octagons.push_back( ngon8_levels );
                        //break;
                    }
                else if(num_fcs == 9){//case 9:
                        store(put_n_gon<9>(msh,cl), m_enneagons);
                        std::array<size_t, 3> ngon9_levels = {cl_level, cl.get_id(), bin_mark};
                        l_enneagons.push_back(ngon9_levels );
                        //break;
                    }
                else if(num_fcs == 10){//case 10:
                        store(put_n_gon<10>(msh,cl), m_decagons);
                        std::array<size_t, 3> ngon10_levels = {cl_level, cl.get_id(), bin_mark};
                        l_decagons.push_back(ngon10_levels );
                        //break;
                    }
                else if(num_fcs == 11){//case 11:
                        store(put_n_gon<11>(msh,cl), m_hendecagons);
                        std::array<size_t, 3> ngon11_levels = {cl_level, cl.get_id(), bin_mark};
                        l_hendecagons.push_back(ngon11_levels );
                        //break;
                    }
                else if(num_fcs == 12){//case 12:
                        store(put_n_gon<12>(msh,cl), m_dodecagons);
                        std::array<size_t, 3> ngon12_levels = {cl_level, cl.get_id(), bin_mark};
                        l_dodecagons.push_back(ngon12_levels );
                        //break;
                    }
                else if(num_fcs == 13){//case 13:
                        store(put_n_gon<13>(msh,cl), m_triadecagons);
                        std::array<size_t, 3> ngon13_levels = {cl_level, cl.get_id(), bin_mark};
                        l_triadecagons.push_back(ngon13_levels );
                        //break;
                    }
                else if(num_fcs == 14){//case 14:
                        store(put_n_gon<14>(msh,cl), m_tesseradecagons);
                        std::array<size_t, 3> ngon14_levels = {cl_level, cl.get_id(), bin_mark};
                        l_tesseradecagons.push_back(ngon14_levels );
                        //break;
                    }
                else if(num_fcs == 15){//case 15:
                        store(put_n_gon<15>(msh,cl), m_pentadecagons);
                        std::array<size_t, 3> ngon15_levels = {cl_level, cl.get_id(), bin_mark};
                        l_pentadecagons.push_back(ngon15_levels );
                        //break;
                    }
                else if(num_fcs == 16){//case 16:
                        store(put_n_gon<16>(msh,cl), m_16gons);
                        std::array<size_t, 3> ngon16_levels = {cl_level, cl.get_id(), bin_mark};
                        l_16gons.push_back(   ngon16_levels );
                        //break;
                    }
                else if(num_fcs == 17){//case 17:
                        store(put_n_gon<17>(msh,cl), m_17gons);
                        std::array<size_t, 3> ngon17_levels = {cl_level, cl.get_id(), bin_mark};
                        l_17gons.push_back( ngon17_levels );
                        //break;
                    }
                else if(num_fcs == 18){//case 18:
                        store(put_n_gon<18>(msh,cl), m_18gons);
                        std::array<size_t, 3> ngon18_levels = {cl_level, cl.get_id(), bin_mark};
                        l_18gons.push_back( ngon18_levels );
                        //break;
                    }
                else if(num_fcs == 19){//case 19:
                        store(put_n_gon<19>(msh,cl), m_19gons);
                        std::array<size_t, 3> ngon19_levels = {cl_level, cl.get_id(), bin_mark};
                        l_19gons.push_back(  ngon19_levels );
                        //break;
                    }
                else if(num_fcs == 20){//case 20:
                        store(put_n_gon<20>(msh,cl), m_20gons);
                        std::array<size_t, 3> ngon20_levels = {cl_level, cl.get_id(), bin_mark};
                        l_20gons.push_back( ngon20_levels );
                        //break;
                    }
                else if(num_fcs == 21){//case 21:
                        store(put_n_gon<21>(msh,cl), m_21gons);
                        std::array<size_t, 3> ngon21_levels = {cl_level, cl.get_id(), bin_mark};
                        l_21gons.push_back(ngon21_levels );
                        //break;
                    }
                else if(num_fcs == 22){//case 22:
                        store(put_n_gon<22>(msh,cl), m_22gons);
                        std::array<size_t, 3> ngon22_levels = {cl_level, cl.get_id(), bin_mark};
                        l_22gons.push_back(ngon22_levels );
                        //break;
                    }
                else if(num_fcs == 23){//case 23:
                        store(put_n_gon<23>(msh,cl), m_23gons);
                        std::array<size_t, 3> ngon23_levels = {cl_level, cl.get_id(), bin_mark};
                        l_23gons.push_back( ngon23_levels );
                        //break;
                    }
                else if(num_fcs == 24){//case 24:
                        store(put_n_gon<24>(msh,cl), m_24gons);
                        std::array<size_t, 3> ngon24_levels = {cl_level, cl.get_id(), bin_mark};
                        l_24gons.push_back(ngon24_levels );
                        //break;
                    //case 25:
                    }
                else if(num_fcs == 25){
                        store(put_n_gon<25>(msh,cl), m_25gons);
                        std::array<size_t, 3> ngon25_levels = {cl_level, cl.get_id(), bin_mark};
                        l_25gons.push_back(ngon25_levels );
                        //break;
                    }
                    //default:
                    else
        ...
        new_binning_marks = std::vector<size_t>(level_ancestor.size());
        for(size_t i = 0; i < level_ancestor.size(); i++)
        {
            size_t idx = loader.m_index_transf.at(i);
            auto pair  = *std::next(level_ancestor.begin(), idx);
            new_levels.at(i) = level_ancestor.at(idx)[0];
            ancestors.at(i)  = level_ancestor.at(idx)[1];
            new_binning_marks.at(i) = level_ancestor.at(idx)[2];
        }
        ...
        std::cout << "Binning_marks" << std::endl;
        size_t c = 0;
        for(auto& l : new_binning_marks)
            std::cout<<" cell "<< c++ << "  = "<< l <<std::endl;
        std::cout<< std::endl;
...
public:
            ...
            const size_t m_degree)
    {
        ...
        dump_to_matlab(msh, "viscoplasticity/circular/Meeting_12_05/k_"+ tostr(m_degree)+ "/test/borrar_initial_marks.m", initial_marks);
        auto node_state_marks = nodes_state(msh, initial_marks);
        ...
        dump_to_matlab(msh, "viscoplasticity/circular/Meeting_12_05/k_"+ tostr(m_degree)+ "/test/mesh_bin_marks.m", ret);
    ...
    find_geometry(const mesh_type& msh, const std::vector<size_t>& cells_marks)
    {
        ...
        std::cout << "binning_marks in find_geometry: " << std::endl;
        size_t clcont = 0;
        for(auto& mark : cells_marks)
        {
            if( mark == 3)
                std::cout << "  "<< clcont;
            clcont++;
        ...
        for(auto& cell : msh)
        {
            ...
            if(!is_checked && cells_marks.at(cell_id))
            {
                ...
                bgeo.cellslevels.push_back(10000);
    ...
    erase_geometry(const mesh_type& msh, mesh_type& new_msh)
    {
        ...
        for(auto cid: bgeo_all.cells2erase)
        {
            ...
            temp_levels.erase(temp_levels.begin() + cid);
        }
    ...
    add_new_cells(const mesh_type& msh, mesh_type& new_msh)
    {
        ...
        for(auto& cl: bgeo_all.cells2bin)
        {
            ...
            temp_levels.push_back(bgeo_all.cellslevels.at(count_level++));
    ...
    bin(const mesh_type& msh)
    {
        ...
        bin(const mesh_type& msh)
        {
            ...
            new_storage->edges_owners.resize(new_storage->edges.size());
            new_storage->edges_owners = faces_owners(new_msh);

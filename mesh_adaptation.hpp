...
void
save_data(const std::vector<punctual_tensor<MeshType>>& vec, const std::string& filename)
{
...
public:
                        ...
                        imsh(adaptive_step)
    {
        ...
        // Marking all cells was conceive as a test
        if(mp.mark_all)
            ...
            const T& yield)
    {
        ...
        for(size_t i = 0; i < tsr_vec.size();i++)
        {
            auto quad_degree = tsr_vec.at(i).sigma.m_quad_degree;
    ...
    gauss_points_positions(const mesh_type& msh, const cell_type& cell,
                            const face_type& face, const size_t& fq_size)
    {
        ...
        //std::cout << " **  pos   : "<< fq_size * fc_pos << std::endl;

        return fq_size * fc_pos;
    }
    dynamic_matrix<T>
...
//#if 0
    ...
    T
    normal_factor(const mesh_type& msh,
                  const cell_type& cl,
                  const face_type& fc)
    {
            ...
            const std::vector<matrix_type>& grad_global)
    {
        ...
        for(auto& cell : msh)
        {
            ...
            //std::cout << "CELL : "<<  msh.lookup( cell )<< std::endl;
            ...
            for( auto& face : c_faces)
            {
                ...
                auto c_fqs_size = c_tsr.sigma.face_qps_cols();
                auto n_fqs_size = n_tsr.sigma.face_qps_cols();
                ...
                auto n_num_faces = points(msh, neighbor).size();

                assert ((c_fqs_size/num_faces) == fqs.size());
                assert ((n_fqs_size/n_num_faces) ==  fqs.size());


                auto cgp_id    =  gauss_points_positions(msh,cell,face, fqs.size());
                auto ngp_id    =  gauss_points_positions(msh,neighbor,face, fqs.size());//Deben tener el mismo tamano en ambas celdas
                ...
                matrix_type c_tau  = c_tsr.sigma.face_quad_pts - pst.alpha * c_tsr.gamma.face_quad_pts;
                matrix_type n_tau  = n_tsr.sigma.face_quad_pts - pst.alpha * n_tsr.gamma.face_quad_pts;
                ...
                for (auto& qp : fqs)
                {
                    ...
                    vector_type c_str  = c_tau.col(cgp_id) + pst.alpha *  c_dphi_ruh;
                    vector_type n_str  = n_tau.col(ngp_id) + pst.alpha * nc_dphi_ruh;
            ...
            auto c_tau =  c_tsr.sigma - pst.alpha * c_tsr.gamma;
        ...
        for(size_t cl_id = 0; cl_id < msh.cells_size(); cl_id++)
        {
            ...
            auto eT1 = eta_stress.at(cl_id);//iexp_pow(eta_stress.at(cl_id), 2.);
            auto eT2 = eta_residual.at(cl_id); //iexp_pow(eta_residual.at(cl_id), 2.);
            set_error += eT;
        ...
        std::cout << "3. BEGIN ETA" << std::endl;
        std::cout << "4.** size LEVELS: "<< levels.size() << std::endl;
            ...
            if( hgi.has_hng_nodes)
            {
                ...
                for(auto fc : fcs)
                {
                    ...
                    if(hgi.faces_has_hang_nodes.at(fcont) == 0)
                    {
                        ...
                        if(msh.is_boundary(fc))
                        {
                            std::cout << "---marking boundary edge" << std::endl;
                            face_mark_hanging_nodes(msh, fc);
                        }
                        else
                        {

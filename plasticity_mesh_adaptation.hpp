...
public:
                      ...
                      bool& do_refinement)
    {
        ...
        cells_marks = vec;
        do_refinement = false;
        for(size_t i = 0; i < cells_marks.size(); i++)
        {
            std::cout<<"cells_marks["<<i<<"]"<<cells_marks[i]<<std::endl;
            if(cells_marks[i] == 1)
            {
                do_refinement = true;
                break;
...
public:
    ...
    stress_based_mesh(const mesh_type& msh,
                        ...
                        const std::string& directory,
                        const std::string& info):m_hanging_nodes(hanging_nodes)
    {
        levels = levels_vec;
        temp_levels.reserve(msh.cells_size());
        marker(msh,tsr_vec,yield,directory,info);
        for(size_t cm = 0; cm < cells_marks.size(); cm++)
        {
            size_t b = cells_marks.at(cm);
            std::cout<<b<<"  ";
        }
        std::cout<<std::endl;
    ...
    check_levels(const mesh_type& msh, const cell_type & cl)
    {
        ...
        for(auto& fc : fcs)
        {
            if(!(msh.is_boundary(fc)))
            {
                ...
                std::cout << "         ngh   :"<<  ngh_id << std::endl;
                std::cout << "         level : "<< ngh_level << std::endl;

                if( std::abs(int(ngh_level - cl_level)) >=2)
                {
                    std::cout << "INSIDE!!!   dif = "<<std::abs(int(ngh_level - cl_level)) << std::endl;
                    if(ngh_level < cl_level)
                    {
                        ++levels.at(ngh_id);
                        cells_marks.at(ngh_id) = 3;
                        auto  ngh  = *(msh.cells_begin() + size_t(ngh_id));
                        check_levels(msh , ngh);
   ...
   template<typename U, size_t N, size_t M>
    ...
    face_marker(mesh_type& msh)
    {
        ...
        {
            auto cl_id   = msh.lookup(cl);
            bool cl_mark = cells_marks.at(cl_id);
            if(cl_mark)
            {
                if(m_hanging_nodes)

...
public:
    ...
    punctual_tensor(const mesh_type& msh, const cell_type& cl, const size_t& quad_degree):m_quad_degree(quad_degree)
    {
        ...
        grid_pts_size = fcs.size();
        face_qps_size = grid_pts_size * fq.integrate(msh, fcs[0]).size(); //WK: This should change if k different for each face
        cell_qps_size = cq.integrate(msh, cl).size();
    };
...
public:
                  ...
                  vector_type     & sigma)
    {
        ...
        vector_type xi = sigma  +  pst.alpha * dphi_ruh;
                 ...
                 tensors<mesh_type> & tsr)
    {
        ...
        for(size_t i = 0;  i < fcs.size(); i++)
        {
            ...
            auto pair  =  eval_plasticity(pts.at(i), msh, cl, ruh, pst, v_sigma);
        ...
        for(size_t i = 0; i < pts.size(); i++)
        {
            ...
            auto pair  = eval_plasticity(fbar, msh, cl, ruh, pst, fb_sigma);
        ...
        auto pair  =  eval_plasticity(cbar, msh, cl, ruh, pst, cb_sigma);
                 ...
                 tensors<mesh_type> & tsr)
    {
        ...
        for (auto& qp : cqs)
        {
            ...
            auto pair  =  eval_plasticity(qp.point(), msh, cl, ruh, pst, sigma);
        ...
        for (size_t face_i = 0; face_i < num_faces; face_i++)
        {
            ...
            for (auto& qp : fqs)
            {
                ...
                auto pair  =  eval_plasticity(qp.point(), msh, cl, ruh, pst, f_sigma);
                ...
                scalar_type p1  = make_prod_stress_n( sigma_agamma, n);
                if(pst.hho == 2)
                {

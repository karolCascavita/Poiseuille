...
public:
    ...
    stress_based_mesh(const mesh_type& msh,const std::vector<tensors<T>>& tsr_vec, const T yield, bool& do_refinement)
    {
        ...
        for(size_t i = 0; i < tsr_vec.size();i++)
        {
            ...
            //#endif
            #if 0
            if(mat.maxCoeff() >= yield )
            {
                if(mat.minCoeff() < yield )
                    vec.at(i) = 1;
            }
            #endif
        }
                ...
                for(auto& fc : mark_faces)
                {
                    ...
                    else
                    {
                        ...
                        if(!fc_marks_vector.at(fc_id).first)
                        {
                            ...
                            //WK: Activar esto!! puesto que es lo que permite que sea recursivo para buscar en los vecinos
                            // en caso de que se escoja adapar tambien los vecinos
   ...
   put_n_gon_with_hanging_node(mesh_type & msh, const cell_type& cl)
   {
                                        ...
                                        const IdxVector& vertices)
    {
        ...
        for(auto & v:vertices)
        {
            ...
            {
                std::cout << "DO NEW BAR" << std::endl;

                auto fc_id  = fcs_ids.at(v);
                auto bar_id = fc_marks_vector.at(fc_id).second;
                ...
                for(auto& v : owner_cells)
                    std::cout<<"            "<< v.id <<" " <<v.cells.first<<" " <<v.cells.second<<" " <<v.point_id<<" "<<v.face_id<<std::endl;
            ...
            else
            {
                ...
                std::cout << "bar_distance" << b_dist<< std::endl;
                ...
                for(size_t ip = v; ip < r_limit; ip++)
                {
                    ...
                    std::cout << "distance" <<std::fixed << std::setprecision(12)<< dist<< std::endl;
                    ...
                    auto fc_mark = fc_marks_vector.at(fc_id).first;
                    if(fc_mark)
                    {
                        npf.id       = ip + j + 1;
                        npf.face_id  = fc_id;
                        npf.point_id = fc_marks_vector.at(fc_id).second;
        ...
        for(auto& v : owner_cells)
        {
            auto pt_id = v.point_id;
            std::cout<<"    point( "<< pt_id <<") = "<<  storage ->points.at(pt_id) <<std::endl;
        }
